...
//  whitespaces and the array is printed as a string of digits and all we know is 
...
function restoreArr(str, k) {
    const splitStr = str.split('');
    const result = new Array(str.length + 1).fill(0);
    ...
    for (let i = str.length - 1; i >= 0; i--) {
        ...
        while (j <= str.length && strResult * 1 <= k) {
            result[i] += result[j];
            
            result[i] %= mod;
            
            if (j < str.length) {
                strResult += splitStr[j];
            }
            j++;
        }
    }
