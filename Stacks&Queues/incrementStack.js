...
// void inc(int k, int val) Increments the bottom k elements of the stack by val. If 
// there are less than k elements in the stack, just increment all the elements in the 
// stack.
...
class StackWIncrement {
    ...
    increment(numToInc, incrementVal) {
        let len = numToInc <= this.data.length ? numToInc : this.data.length;
        
        for (let i = 0; i < len; i++) {
            this.data[i] += incrementVal;
        }
